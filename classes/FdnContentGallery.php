 ...
 */
class FdnContentGallery extends \ContentElement
{
	...
	protected function compile()
	{
		...
		$objTemplate->fdnBlockClass = '';
		$objTemplate->fdnAttributes = '';

		if($this->useFoundationBlockGrid)
		{
			...
			$objTemplate->fdnBlockClass  = 'row small-up-'.$this->fdn_smallBlock.' '.'medium-up-'.$this->fdn_mediumBlock.' '.'large-up-'.$this->fdn_largeBlock;
			
			if(!empty($this->fdn_classes)) {
				$objTemplate->fdnBlockClass .= ' '.trim($this->fdn_classes);
			}

			if ($this->fdn_equalizer)
			{
				$objTemplate->fdnAttributes .= 'data-equalizer-watch';
			}

			if (!empty($this->fdn_attributes))
			{
				$objTemplate->fdnAttributes .= (empty($this->Template->fdnAttributes) ? '': ' ').trim($this->fdn_attributes);
			}
		}
